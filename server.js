...
app.post('/api/generate-questions', async (req, res) => {
  const { subjectName, scheme, kind, count } = req.body || {};
  ...
  try {
    const prompt = `You are setting a Nigerian secondary school ${kind === 'test' ? 'class test' : 'examination'} for the subject "${subjectName}", based strictly on the scheme of work below.
Write exactly ${count} multiple-choice questions, each with 4 options (A-D) and exactly one correct option, at an appropriate secondary-school difficulty. Avoid repeating the same question idea twice.
Scheme of work:
"""
${scheme}
"""
